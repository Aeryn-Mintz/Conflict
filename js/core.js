...
let audioCtx = null, soundboardDest = null, soundboardTrack = null;
const peers = {}, peerNames = {}, peerAvatars = {}, iceCandidateQueue = {};
...
let localMicStream = null, localScreenStream = null, localCamStream = null;
...
function connectSocket() {
    ...
    window.socket.onopen = () => {
        ...
        if (joinBtn && !localMicStream) { joinBtn.disabled = false; joinBtn.innerText = "🎙️ Join Call"; }
        document.getElementById('chat-input').disabled = false; document.getElementById('chat-send-btn').disabled = false; document.getElementById('chat-input').placeholder = '🟢 Connected! Message the room...';
    ...
    window.socket.onclose = () => {
        reconnectAttempts++; const delaySec = Math.min(30, Math.pow(2, reconnectAttempts));
        document.getElementById('chat-input').placeholder = `🟡 Disconnected — reconnecting in ${delaySec}s...`;
        document.getElementById('chat-input').disabled = true; document.getElementById('chat-send-btn').disabled = true;
    ...
    window.socket.onmessage = async (event) => {
        try {
            ...
            if (data.action === 'join' && data.userId !== window.myId) {
                peerNames[data.userId] = data.username || "Guest"; peerAvatars[data.userId] = data.avatar || null;
                addRemoteUserCard(data.userId, peerNames[data.userId], peerAvatars[data.userId]);
                let pc = peers[data.userId] || createPeerConnection(data.userId); addLocalTracksSafely(pc);
                ...
                if (window.isDM && window.viewingParty) {
                    if (document.getElementById('party-list-grid')) window.renderPartyList();
                    if (window.viewingRemoteUid === data.userId) window.renderRemoteSheet(data.userId);
                }
            }
            else if (data.action === 'offer' && data.targetId === window.myId) {
                if (data.username) peerNames[data.userId] = data.username;
                if (data.avatar) peerAvatars[data.userId] = data.avatar;
                addRemoteUserCard(data.userId, peerNames[data.userId], peerAvatars[data.userId]);
                let pc = peers[data.userId] || createPeerConnection(data.userId);
                await pc.setRemoteDescription(new RTCSessionDescription(data.offer));
                if (iceCandidateQueue[data.userId]) { while (iceCandidateQueue[data.userId].length > 0) await pc.addIceCandidate(iceCandidateQueue[data.userId].shift()); }
            ...
            else if (data.action === 'answer' && data.targetId === window.myId && peers[data.userId]) await peers[data.userId].setRemoteDescription(new RTCSessionDescription(data.answer));
            else if (data.action === 'ice-candidate' && data.targetId === window.myId && peers[data.userId]) {
                const pc = peers[data.userId]; const candidate = new RTCIceCandidate(data.candidate);
                if (pc.remoteDescription && pc.remoteDescription.type) await pc.addIceCandidate(candidate);
                else { if (!iceCandidateQueue[data.userId]) iceCandidateQueue[data.userId] = []; iceCandidateQueue[data.userId].push(candidate); }
            }
            ...
            else if (data.action === 'yt_load') { ytQueue.unshift({ id: data.videoId }); loadAndPlayVideo(data.videoId, 0); window.addChatLine('System', `▶️ Playing new YouTube Video`, true); }
            else if (data.action === 'yt_queue_add') { ytQueue.push({ id: data.videoId }); renderQueueUI(); window.addChatLine('System', `➕ A user added a video to the queue`, true); }
            else if (data.action === 'yt_skip' || data.action === 'yt_jump') { currentQueueIndex = data.action === 'yt_jump' ? data.index : currentQueueIndex + 1; if (ytQueue[currentQueueIndex]) loadAndPlayVideo(ytQueue[currentQueueIndex].id, currentQueueIndex); }
            else if (data.action === 'yt_pause') { if (ytPlayer && ytPlayer.getPlayerState() !== YT.PlayerState.PAUSED) ytPlayer.pauseVideo(); }
            else if (data.action === 'yt_play') { if (ytPlayer && ytPlayer.getPlayerState() !== YT.PlayerState.PLAYING) ytPlayer.playVideo(); }
...
function addLocalTracksSafely(pc) {
    ...
    tracksToAdd.forEach(track => { if (!sentTracks.includes(track)) pc.addTrack(track, localMicStream || localScreenStream || localCamStream || soundboardDest.stream); });
}
...
function createPeerConnection(remoteUserId) {
    ...
    pc.oniceconnectionstatechange = () => {
        if (['disconnected', 'failed', 'closed'].includes(pc.iceConnectionState)) {
            ...
            delete peers[remoteUserId]; delete iceCandidateQueue[remoteUserId];
        }
    };
    pc.ontrack = (event) => {
        ...
        if (event.track.kind === 'video') {
            ...
            if (!videoContainer) {
                ...
                const label = document.createElement('div'); label.className = "stream-label"; label.innerText = `${peerNames[remoteUserId] || 'Friend'}'s Video`;
...
function addRemoteUserCard(userId, username, avatarDataUrl) {
    ...
    if (!card) {
        ...
        card.innerHTML = `<div class="avatar-wrapper"><img src="${avatarDataUrl || 'https://via.placeholder.com/50'}" class="avatar" alt="Avatar"></div><div class="user-info"><span class="username">${username}</span><input type="range" class="volume-slider" id="vol-${userId}" min="0" max="1" step="0.05" value="0.8"></div>`;
...
document.getElementById('join-voice-btn')?.addEventListener('click', async () => {
    ...
    try {
        if (!audioCtx) { audioCtx = new (window.AudioContext || window.webkitAudioContext)(); soundboardDest = audioCtx.createMediaStreamDestination(); soundboardTrack = soundboardDest.stream.getAudioTracks()[0]; }
        if (audioCtx.state === 'suspended') await audioCtx.resume();
        const micId = localStorage.getItem('appAudioIn');
        const constraints = micId ? { audio: { deviceId: { exact: micId }, echoCancellation: true, noiseSuppression: true } } : { audio: { echoCancellation: true, noiseSuppression: true } };
        localMicStream = await navigator.mediaDevices.getUserMedia(constraints); micSuccess = true;
...
document.getElementById('mute-mic-btn')?.addEventListener('click', (e) => {
    if (localMicStream) {
        isMuted = !isMuted; localMicStream.getAudioTracks()[0].enabled = !isMuted;
        e.target.innerText = isMuted ? "🔈 Unmute" : "🔇 Mute"; e.target.style.color = isMuted ? "var(--accent-alert)" : "var(--text-main)"; e.target.style.borderColor = isMuted ? "var(--accent-alert)" : "var(--border-color)";
    }
});
...
document.getElementById('toggle-cam-btn')?.addEventListener('click', async (e) => {
    ...
    if (localCamStream) {
        if (window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'remove_video', streamId: localCamStream.id, trackId: localCamStream.getVideoTracks()[0]?.id }));
        localCamStream.getTracks().forEach(t => { t.stop(); Object.values(peers).forEach(pc => { const sender = pc.getSenders().find(s => s.track === t); if (sender) pc.removeTrack(sender); }); });
        localCam.style.display = 'none'; localCam.srcObject = null; localCamStream = null;
        ...
        try {
            ...
            localCam.style.display = 'block'; localCam.srcObject = localCamStream;
...
document.getElementById('share-screen-btn')?.addEventListener('click', async (e) => {
    ...
    if (localScreenStream) {
        if (window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'remove_video', streamId: localScreenStream.id, trackId: localScreenStream.getVideoTracks()[0]?.id }));
        localScreenStream.getTracks().forEach(t => t.stop());
        if (localScreenStream.getVideoTracks().length > 0) localScreenStream.getVideoTracks()[0].dispatchEvent(new Event('ended'));
        return;
    }
    try {
        ...
        localVideo.style.display = 'block'; localVideo.srcObject = localScreenStream; localVideo.controls = true; 
        for (const [remoteUserId, pc] of Object.entries(peers)) {
            addLocalTracksSafely(pc);
            try { const offer = await pc.createOffer(); await pc.setLocalDescription(offer); if (window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'offer', userId: window.myId, targetId: remoteUserId, offer: offer })); } catch (err) {}
        }
        localScreenStream.getVideoTracks()[0].onended = async () => {
            ...
            for (const [remoteUserId, pc] of Object.entries(peers)) {
                pc.getSenders().forEach(sender => { if (sender.track && sender.track.kind === 'video' && !localCamStream?.getTracks().includes(sender.track)) pc.removeTrack(sender); });
...
document.getElementById('cancel-screen-picker')?.addEventListener('click', () => { window.ipcRenderer.send('screen-picker-result', null); document.getElementById('screen-picker-modal').style.display = 'none'; document.getElementById('modal-overlay').style.display = 'none'; });

// YouTube 
let ytPlayer = null; let pendingYtVideoId = null; let ytQueue = []; let currentQueueIndex = -1;
function extractYouTubeId(url) {
    ...
    try {
        ...
        if (host === 'youtu.be') id = u.pathname.split('/')[1];
            ...
            if (u.pathname === '/watch') id = u.searchParams.get('v'); else { const match = u.pathname.match(/^\/(embed|shorts|live)\/([^\/]+)/); if (match) id = match[2]; }
        }
        return (id && /^[A-Za-z0-9_-]{11}$/.test(id)) ? id : null;
    } catch (e) { return null; }
}
const ytApiTag = document.createElement('script'); ytApiTag.src = 'https://www.youtube.com/iframe_api'; document.head.appendChild(ytApiTag);
window.onYouTubeIframeAPIReady = function () {
    ytPlayer = new YT.Player('yt-iframe', { height: '100%', width: '100%', playerVars: { autoplay: 1, origin: window.location.origin }, events: {
        onReady: () => { if (pendingYtVideoId) { loadAndPlayVideo(pendingYtVideoId); pendingYtVideoId = null; } },
        onStateChange: (event) => { if (event.data === YT.PlayerState.ENDED) playNextInQueue(); },
        onError: () => { window.addChatLine('System', "⚠️ That video can't be played here.", true); playNextInQueue(); }
    }});
};
function loadAndPlayVideo(videoId, index = -1) {
    if (!videoId) return;
    document.getElementById('yt-wrapper').style.display = 'flex'; if (index !== -1) currentQueueIndex = index;
    if (ytPlayer && typeof ytPlayer.loadVideoById === 'function') ytPlayer.loadVideoById(videoId); else pendingYtVideoId = videoId;
    renderQueueUI();
}
function queueVideo(videoId, broadcast = true) {
    if (!videoId) return;
    ytQueue.push({ id: videoId, title: `Video (${videoId})` });
    if (currentQueueIndex === -1) playNextInQueue(); else renderQueueUI();
...
function playNextInQueue() { if (ytQueue.length === 0) return; currentQueueIndex++; if (currentQueueIndex >= ytQueue.length) currentQueueIndex = 0; loadAndPlayVideo(ytQueue[currentQueueIndex].id, currentQueueIndex); }
function renderQueueUI() {
    const listEl = document.getElementById('yt-queue-list'); const countEl = document.getElementById('queue-count');
    if (!listEl) return;
    listEl.innerHTML = ''; countEl.innerText = `${ytQueue.length} items`;
    ytQueue.forEach((item, index) => {
        const row = document.createElement('div'); row.className = index === currentQueueIndex ? 'yt-queue-item active-queue-item' : 'yt-queue-item';
        row.innerHTML = `<span>${index + 1}. ${item.id}</span><div class="yt-queue-controls"><button class="yt-queue-btn" onclick="jumpToQueue(${index})" title="Play Now">▶️</button><button class="yt-queue-btn" onclick="removeFromQueue(${index})" title="Remove">✕</button></div>`;
        listEl.appendChild(row);
    });
}
window.jumpToQueue = function(index) { currentQueueIndex = index; loadAndPlayVideo(ytQueue[index].id, index); if (window.socket && window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'yt_jump', index: index })); };
window.removeFromQueue = function(index) { ytQueue.splice(index, 1); if (index === currentQueueIndex) playNextInQueue(); else if (index < currentQueueIndex) currentQueueIndex--; renderQueueUI(); };

document.getElementById('yt-queue-add-btn')?.addEventListener('click', () => { const vidId = extractYouTubeId(document.getElementById('yt-url-input').value); if (vidId) { queueVideo(vidId, true); document.getElementById('yt-url-input').value = ''; window.addChatLine('System', `➕ Added video to queue`, true); } else alert("Invalid Link!"); });
document.getElementById('yt-play-btn')?.addEventListener('click', () => { const vidId = extractYouTubeId(document.getElementById('yt-url-input').value); if (vidId) { ytQueue.unshift({ id: vidId, title: `Video (${vidId})` }); loadAndPlayVideo(vidId, 0); if (window.socket && window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'yt_load', videoId: vidId })); document.getElementById('yt-url-input').value = ''; } else alert("Invalid Link!"); });
document.getElementById('yt-skip-btn')?.addEventListener('click', () => { playNextInQueue(); if (window.socket && window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'yt_skip' })); });
...
document.getElementById('yt-playpause-btn')?.addEventListener('click', () => { if (!ytPlayer || typeof ytPlayer.getPlayerState !== 'function') return; if (ytPlayer.getPlayerState() === YT.PlayerState.PLAYING) { ytPlayer.pauseVideo(); if (window.socket && window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'yt_pause' })); } else { ytPlayer.playVideo(); if (window.socket && window.socket.readyState === WebSocket.OPEN) window.socket.send(JSON.stringify({ action: 'yt_play' })); } });
...
document.getElementById('add-sound-upload')?.addEventListener('change', (event) => {
    ...
    tempAudio.onloadedmetadata = () => {
        ...
        btn.onclick = () => {
            ...
            if (audioCtx) { const source = audioCtx.createMediaElementSource(effectAudio); const gainNode = audioCtx.createGain(); gainNode.gain.value = Math.min(parseFloat(document.getElementById('effectsVolumeSlider').value), 1); source.connect(gainNode); gainNode.connect(audioCtx.destination); gainNode.connect(soundboardDest); }
            effectAudio.play();
...
document.getElementById('chat-send-btn')?.addEventListener('click', () => {
    const text = document.getElementById('chat-input').value.trim(); if (!text || !window.socket || window.socket.readyState !== WebSocket.OPEN) return;
    document.getElementById('chat-input').value = ''; const name = document.getElementById('display-username').textContent;
...
document.getElementById('audio-out-select')?.addEventListener('change', (e) => { localStorage.setItem('appAudioOut', e.target.value); applyAudioOutput(e.target.value); });
...
document.getElementById('reset-theme-btn')?.addEventListener('click', () => { localStorage.setItem('appTheme', JSON.stringify(defaultTheme)); loadTheme(); });
